...
export class ContextMenu {
  ...
  private position = (mouseX: number, mouseY: number) => {
    ...
    const outOfBoundsOnX =
      scopeX + this.element.clientWidth > this.scope.clientWidth;

    const outOfBoundsOnY =
      scopeY + this.element.clientHeight > this.scope.clientHeight;
    ...
    if (outOfBoundsOnX) {
      normalizedX =
        scopeOffsetX + this.scope.clientWidth - this.element.clientWidth;
    }

    // ? normalize on Y
    if (outOfBoundsOnY) {
      normalizedY =
        scopeOffsetY + this.scope.clientHeight - this.element.clientHeight;
    }
