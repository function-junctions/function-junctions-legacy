...
export type ValueInRange = (scale: {
  minScale: number;
  maxScale: number;
  scale: number;
}) => boolean;
...
export type GetTranslate = (scale: {
  minScale: number;
  maxScale: number;
  scale: number;
}) => (position: { pos: number; prevPos: number; translate: number }) => number;

export type GetMatrix = (translate: {
  scale: number;
  translateX: number;
  translateY: number;
}) => string;
...
const valueInRange: ValueInRange = ({ minScale, maxScale, scale }) =>
  scale <= maxScale && scale >= minScale;

const getTranslate: GetTranslate =
...
export const getTruePosition: GetTruePosition = ({
  x,
  y,
  scale,
  translateX,
...
const canPan: CanPan = (state: Drag) => ({
  ...
  panTo: ({ originX, originY, scale }) => {
    ...
    pan({
      state,
      originX: originX - transformation.translateX,
      originY: originY - transformation.translateY,
    });
  },
});
